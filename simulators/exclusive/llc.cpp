...
void LLC::insertData(intptr_t addr, int tid, bool isWrite, bool &hit, bool &writeback, intptr_t &evictedAddr, bool updateReplacement, bool actuallyInsert)
{
    ...
    PIN_GetLock(&m_setLocks[setID], tid+1);
    {
        ...
        if (hit == false && actuallyInsert == true)
        {
    ...
    if (hit == true)
    {
      ...
      for (int way = 0; way < m_numWays; ++way)
      {
        ...
        {
            m_tagArray[setID][way] = -1; // invalidate
            moveToLRU(setID, way);
            m_dirty[setID] = m_dirty[setID] & ~(1 << way); 
...
void LLC::moveToLRU (int setID, int wayID)
{
